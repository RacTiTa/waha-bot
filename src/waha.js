...
export const waha = {
  ...
  sendSeen: (chatId, messageId) => call('/api/sendSeen', { chatId, messageId }),
  startTyping: (chatId) => call('/api/startTyping', { chatId }),
  stopTyping: (chatId) => call('/api/stopTyping', { chatId }),
};

/** Marca leído + "escribiendo…" mientras se resuelve la respuesta. Nunca rompe el flujo. */
export async function withTyping(chatId, messageId, fn) {
  const quiet = (p) => p.catch((err) => console.warn('[waha]', err.message));
